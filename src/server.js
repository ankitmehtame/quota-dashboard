...
import { DEFAULT_CONFIG, PROVIDER_IDS, USAGE_SOURCE_IDS, localDateRange, normalizeConfig, providerStatus } from "./lib/core.js";
...
async function getQuota(id, force = false) {
...
async function dashboard(url, config) {
  const range = localDateRange(url.searchParams.get("days") || 30, url.searchParams.get("timezone") || undefined, url.searchParams.get("range") || "relative");
...
async function handleApi(request, response, url) {
  ...
  if (providerMatch && PROVIDER_IDS.includes(providerMatch[1])) {
  ...
  if (usageMatch && USAGE_SOURCE_IDS.includes(usageMatch[1]) && request.method === "PUT") {
    const input = await body(request);
    if (typeof input.enabled !== "boolean") return json(response, 400, { error: "enabled must be boolean" });
    config.usageSources[usageMatch[1]].enabled = input.enabled;
    await saveConfig(config);
    dashboardCache.clear();
    return json(response, 200, { source: usageMatch[1], enabled: input.enabled });
  }

...
function quotaNowPosition(window) {
...
function quotaCard(id, provider, quota) {
  const window = quota?.windows?.[0];
  const percent = window?.usedPercent;
  const value = percent === null || percent === undefined
    ? (window?.balanceLabel ? `<span class="quota-balance-primary">${window.balanceLabel}</span><span class="quota-balance-secondary">${window.spentLabel || ""}</span>` : window?.valueLabel || "Not available")
    : `${Math.round(percent)}%`;
...
function renderUsage(usage) {
  ...
  const sourceNames = { codex: "Codex", opencode: "OpenCode", hermes: "Hermes", shared: "Shared" };
  const sourceColors = { codex: "mint", opencode: "violet", hermes: "orange", shared: "blue" };
  ...
  const renderSegment = (day, segment, height, offset = 0) => { const color = colors[segment.provider] || "mint"; return `<div class="chart-segment ${color}" style="height:${height}%;bottom:${offset}%"><span class="chart-tooltip"><strong>${day.date}</strong><span><i class="tooltip-harness-dot ${color}"></i>${segment.provider} · ${money(segment.costUsd)}</span><span>${(segment.totalTokens || 0).toLocaleString()} tokens</span></span></div>`; };
  const todayOnly = usage.from === usage.to;
  chart.innerHTML = daily.length ? daily.map((day, dayIndex) => { const segments = providers.map((provider) => ({ provider, ...(day.byProvider?.[provider] || {}) })).filter((segment) => segment.costUsd > 0 || segment.totalTokens > 0); const fallback = segments.length ? segments : [{ provider: "other", costUsd: day.costUsd, totalTokens: day.totalTokens }]; const displayedTotal = fallback.reduce((total, segment) => total + segment.costUsd, 0); if (todayOnly) return fallback.map((segment) => `<div class="chart-column today-harness" data-day-index="${dayIndex}" tabindex="0" aria-label="${day.date} ${segment.provider}: ${money(segment.costUsd)}"><div class="chart-stack"><div class="chart-segment ${colors[segment.provider] || "mint"}" style="height:${max ? Math.max(2, (segment.costUsd / max) * 100) : 2}%;bottom:0"><span class="chart-tooltip"><strong>${day.date}</strong><span><i class="tooltip-harness-dot ${colors[segment.provider] || "mint"}"></i>${segment.provider} · ${money(segment.costUsd)}</span><span>${(segment.totalTokens || 0).toLocaleString()} tokens</span></span></div></div></div>`).join(""); const isCurrentDay = day.date === usage.to; let offset = 0; const markup = fallback.map((segment) => { const height = isCurrentDay ? (displayedTotal > 0 ? (segment.costUsd / displayedTotal) * 100 : 0) : (max ? Math.max(2, (segment.costUsd / max) * 100) : 2); const html = renderSegment(day, segment, height, offset); offset += height; return html; }).join(""); const stackHeight = max ? Math.max(2, (displayedTotal / max) * 100) : 2; return `<div class="chart-column ${isCurrentDay && !todayOnly ? "current-day" : ""}" data-day-index="${dayIndex}" tabindex="0" aria-label="${day.date}: ${money(displayedTotal)}"><div class="chart-stack" style="${isCurrentDay && !todayOnly ? `height:${stackHeight}% !important` : ""}">${markup}</div></div>`; }).join("") : `<div class="chart-empty">${usage.error || "No usage data in this range"}</div>`;
  chart.querySelectorAll("[data-day-index]").forEach((bar) => bar.addEventListener("click", () => openDayDetails(daily[Number(bar.dataset.dayIndex)])));
...
async function loadDashboard(refresh = false) {
  ...
  document.querySelectorAll(".range-picker button").forEach((button) => { button.disabled = true; });
  ...
  try {
    ...
    renderQuotas(state.dashboard); renderUsage(state.dashboard.usage); renderStatus(state.dashboard);
  } finally {
    document.querySelector(".range-picker")?.classList.remove("loading");
    document.querySelectorAll(".range-picker button").forEach((button) => { button.disabled = false; });
  }
}
...
async function loadSettings() {
  ...
  const usageSettings = usageSourceOrder.map((id) => `<label class="setting-row"><div class="setting-copy"><div class="provider-name">${usageSourceNames[id]} usage</div><div class="provider-sub">Provider group from shared ccusage output</div></div><input class="switch" type="checkbox" data-kind="usage" data-provider="${id}" ${data.usageSources?.[id]?.enabled ? "checked" : ""} aria-label="Enable ${usageSourceNames[id]} usage" /></label>`).join("");
  ...
  document.querySelectorAll(".switch").forEach((input) => input.addEventListener("change", async (event) => { const id = event.target.dataset.provider; const kind = event.target.dataset.kind; const path = kind === "usage" ? `/api/v1/usage-sources/${id}/enabled` : `/api/v1/providers/${id}/enabled`; await fetch(path, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled: event.target.checked }) }); showToast(`${kind === "usage" ? usageSourceNames[id] + " usage" : data.providers[id].name} ${event.target.checked ? "enabled" : "disabled"}`); await loadDashboard(true); }));
}
...
$("#refresh-button").addEventListener("click", async () => { try { await loadDashboard(true); showToast("Data refreshed"); } catch (error) { showToast(error.message); } });
...
$("#day-details-dialog").addEventListener("close", () => { $("#usage-chart").classList.remove("suppress-tooltips"); document.activeElement?.blur?.(); });
document.querySelectorAll("[data-range]").forEach((button) => button.addEventListener("click", async (event) => { const value = event.currentTarget.dataset.range; state.range = value; state.days = value === "today" ? 1 : value.startsWith("relative-") ? Number(value.slice(9)) : value === "calendar-year" ? 365 : value === "calendar-month" ? 31 : 7; document.querySelectorAll(".range-tab").forEach((tab) => tab.classList.toggle("active", value === "today")); document.querySelectorAll(".range-menu-items button").forEach((item) => item.classList.toggle("active", item.dataset.range === value)); document.querySelectorAll(".range-menu-button").forEach((menuButton) => { const menu = menuButton.parentElement; const selected = menu.querySelector(`[data-range="${value}"]`); menuButton.classList.toggle("active", Boolean(selected)); menuButton.setAttribute("aria-expanded", "false"); menu.querySelector(".range-menu-items")?.classList.remove("open"); }); if (value !== "today") event.currentTarget.closest(".range-menu")?.querySelector(".range-menu-button").firstChild && (event.currentTarget.closest(".range-menu").querySelector(".range-menu-button").childNodes[0].textContent = `${event.currentTarget.textContent} `); await loadDashboard(); }));
document.querySelectorAll(".range-menu-button").forEach((button) => button.addEventListener("click", (event) => { const menu = event.currentTarget.parentElement.querySelector(".range-menu-items"); const open = menu.classList.toggle("open"); event.currentTarget.setAttribute("aria-expanded", String(open)); }));
...
document.addEventListener("pointerdown", (event) => { if (!event.target.closest(".range-picker")) closeRangeMenus(); });
renderClock(); setInterval(renderClock, 30_000); setInterval(() => { if (state.dashboard) renderQuotas(state.dashboard); }, 60_000);
loadDashboard().catch((error) => { $("#status-copy").textContent = error.message; showToast(error.message); });
